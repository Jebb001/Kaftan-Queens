...
export default function CartDrawer() {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetContent className="bg-[hsl(var(--kq-bg))] w-full sm:max-w-md flex flex-col p-0">
        ...
        <div className="flex-1 overflow-auto px-6 py-4">
          {items.length === 0 ? (
            ...
            <ul className="divide-y divide-[hsl(var(--kq-line))]">
              {items.map((it) => (
                <li key={it.key} className="py-4 flex gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-[hsl(var(--kq-ink-soft))]">Size: {it.size}</p>
                    <div className="flex items-center justify-between mt-3">
                      <div className="inline-flex items-center border border-[hsl(var(--kq-line))]">
                        <button className="p-2" onClick={() => updateQty(it.key, it.qty - 1)} aria-label="Decrease">
                          <Minus className="w-3 h-3" />
                        </button>
                        <span className="px-3 text-sm tabular-nums">{it.qty}</span>
                        <button className="p-2" onClick={() => updateQty(it.key, it.qty + 1)} aria-label="Increase">
        ...
        {items.length > 0 && (
          <div className="px-6 py-5 border-t border-[hsl(var(--kq-line))] space-y-4">
            ...
            <Link
              to="/cart"
              onClick={() => setIsOpen(false)}
              className="block text-center w-full bg-[hsl(var(--kq-ink))] text-[hsl(var(--kq-bg))] py-3.5 text-xs tracking-[0.25em] uppercase hover:bg-[hsl(var(--kq-terracotta))] transition-colors"
            >
              Checkout
            </Link>
          </div>
